...
def two_opt_optimization(sol_arr,t,n,min_l):
	...
	if ai != ci and bi != ci:
		...
		if(np.random.random() < p ):
			...
			return new_arr, ai_, bi_, ci_, di_
			
	return sol_arr, ai_, bi_, ci_, di_
...
def sa_algorithm (input_data):
	...
	while t>0.1:
		...
		sol_arr, ai_, bi_, ci_, di_ = two_opt_optimization(sol_arr,t,n,min_l)
		...
		if i>=200 :
			...
			if current_l < min_l:
				...
				plt.plot(frame_arr[:,0],frame_arr[:,1],color='b')
				plt.plot([frame_arr[0][0],frame_arr[-1][0]],[frame_arr[0][1],frame_arr[-1][1]], color='b')
				plt.title('Travelling Salesman Problem\n points ' + ("%d" % n) +' temp=' + ("%f" % t) + '    ' 'curr Length='+ ("%f" % current_l))
				
				# global image_no
				plt.savefig("image" +  str(image_no) + '.png')
				image_no += 1
				plt.clf()
	
	return best_arr, min_l
